...
class DeltaNeutralVolumeBot:
    ...
    def _get_symbol_step_size(self, client: ApiClient, symbol: str) -> Decimal:
        ...
        for symbol_info in symbols:
            ...
            for filter_entry in symbol_info.get("filters", []):
                if filter_entry.get("filterType") == "LOT_SIZE":
                    step_size = Decimal(str(filter_entry.get("stepSize")))
                    if step_size <= 0:
                        raise ValueError(f"Invalid step size {step_size} for symbol {symbol}")
                    self._step_size_cache[symbol] = step_size
                    return step_size
        raise ValueError(f"Unable to determine step size for symbol {symbol}")
    ...
    def _determine_order_quantity(self, account_state: AccountState) -> Decimal:
        if self.config.order_quantity is not None:
            ...
            step_size = self._get_symbol_step_size(account_state.client, self.config.symbol)
            quantity = (self.config.order_notional / price).quantize(step_size, rounding=ROUND_DOWN)
            if quantity <= 0:
                raise ValueError(
