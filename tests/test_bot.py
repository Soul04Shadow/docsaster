...
def test_notional_below_minimum_quantity(monkeypatch: pytest.MonkeyPatch) -> None:
    bot = make_bot()
    bot.config.order_notional = Decimal("1")
    bot._wait_for_fill = lambda *args, **kwargs: {"orderId": 1}
    bot._fetch_trade_records = lambda *args, **kwargs: []

    bot._place_and_wait(bot.long, side="BUY")

    call = bot.long.client.place_order_calls[-1]
...
def test_dry_run_respects_minimum_quantity() -> None:
    bot = make_bot(dry_run=True)
    bot.config.order_notional = Decimal("1")
    ...
    assert trades[0].quote_qty == Decimal("0.001") * Decimal("16345")
