...
def test_load_config(tmp_path: Path) -> None:
    ...
    assert bot_config.order_notional == Decimal("25")
    assert bot_config.order_quantity is None
...
def test_load_config_optional_fields(tmp_path: Path) -> None:
    ...
    assert bot_config.order_notional == Decimal("75.5")


def test_load_config_accepts_order_quantity(tmp_path: Path) -> None:
    config_path = tmp_path / "config.json"
    config_path.write_text(
        """
        {
    ...
    _, _, bot_config = load_config(config_path)
    assert bot_config.order_quantity == Decimal("0.5")
    assert bot_config.order_notional is None


def test_load_config_requires_order_value_or_quantity(tmp_path: Path) -> None:
    config_path = tmp_path / "config.json"
    config_path.write_text(
        """
        {
